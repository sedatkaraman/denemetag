...
from ..helpers import admin, clean_mode, emojiler, bayraklar


@Client.on_message(filters.command(commands="btag", prefixes=COMMAND))
@admin
async def btag(client: Client, message: Message):
    global calisan
    global reasons
    global admins
        ...
        m = await client.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            LAN.TEKLI, callback_data=f"btag 1|{user_id}|{tip}"
                        ),
                        InlineKeyboardButton(
                            LAN.COKLU.format(COUNT),
                            callback_data=f"btag {COUNT}|{user_id}|{tip}",
                        ),
                    ],
                    [
                        InlineKeyboardButton(
                            "❌", callback_data=f"btag 0|{user_id}|{tip}"
                        ),
                    ],
                ],
            ),
...
@Client.on_callback_query(filters.regex(pattern=r"btag"))
async def bcommands(bot: Client, query: CallbackQuery):
    global admins
    global reasons
    global calisan
    q = query.data
    ...
    if int(useer_id) == int(query.from_user.id):
        ...
        started = await bot.send_message(
            ...
            LAN.TAG_START.format(
                query.from_user.mention, "Bayrakla Tag", etiketlenecek, DURATION
            ),
        )
        usrnum = 0
        usrtxt = ""
