...
from ..helpers import admin, clean_mode, emojiler, kartlar


@Client.on_message(filters.command(commands="otag", prefixes=COMMAND))
@admin
async def otag(client: Client, message: Message):
    global calisan
    global reasons
    global admins
    ...
    if chat_id in calisan:
        ...
        m = await client.send_message(
            chat_id,
            LAN.ASK_ISKAMBIL_TAG,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            LAN.TEKLI, callback_data=f"otag 1|{user_id}|{tip}"
                        ),
                        InlineKeyboardButton(
                            LAN.COKLU.format(COUNT),
                            callback_data=f"otag {COUNT}|{user_id}|{tip}",
                        ),
                    ],
                    [
                        InlineKeyboardButton(
                            "❌", callback_data=f"otag 0|{user_id}|{tip}"
                        ),
                    ],
                ],
            ),
...
@Client.on_callback_query(filters.regex(pattern=r"otag"))
async def ocommands(bot: Client, query: CallbackQuery):
    global admins
    global reasons
    global calisan
    q = query.data
    ...
    if int(useer_id) == int(query.from_user.id):
        ...
        started = await bot.send_message(
            ...
            LAN.TAG_START.format(
                query.from_user.mention, "İskambil Kartı ile Etiketleme", etiketlenecek, DURATION
            ),
        )
        usrnum = 0
        usrtxt = ""
